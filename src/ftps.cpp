...
using obn::plat::socket_t;
using obn::plat::kInvalidSocket;

int wait_fd(socket_t fd, short events, int timeout_ms)
{
    return obn::plat::wait_socket(fd, events, timeout_ms);
}
...
socket_t connect_tcp(const std::string& host, int port, int timeout_ms, std::string& err)
{
        ...
        if (e == obn::plat::kEINPROGRESS || e == obn::plat::kEWOULDBLOCK) {
            if (wait_fd(fd, POLLOUT, timeout_ms) > 0) {
                ...
                if (::getsockopt(fd, SOL_SOCKET, SO_ERROR,
                                 reinterpret_cast<char*>(&soerr), &slen) == 0 && soerr == 0)
                    break;
                err = "connect: " + obn::plat::socket_strerror(
                    soerr ? soerr : obn::plat::socket_last_error());
            } else {
                err = "connect: timeout";
            }
        } else {
            err = "connect: " + obn::plat::socket_strerror(e);
        }
        obn::plat::close_socket(fd);
        fd = kInvalidSocket;
    }
    ...
    if (!obn::plat::is_valid_socket(fd) && err.empty()) err = "connect failed";
    int yes = 1;
...
struct Client::Impl {
    ...
    ~Impl()
    {
        ...
        if (obn::plat::is_valid_socket(ctrl_fd)) {
            obn::plat::close_socket(ctrl_fd);
...
std::string Client::connect(const ConnectConfig& cfg)
{
    ...
    if (!obn::plat::is_valid_socket(fd)) return "tcp: " + err;
    p_->ctrl_fd = fd;
...
static socket_t open_data_tcp(Client::Impl& p, const std::string& host, std::string& err)
{
    ...
    if (!obn::plat::is_valid_socket(fd)) err = "data tcp: " + connect_err;
    return fd;
}
                         ...
                         ProgressFn         progress)
{
    ...
    if (!obn::plat::is_valid_socket(data_fd)) return err;
...
std::string Client::list(const std::string& path, std::string& err_out)
{
    std::string err;
    socket_t data_fd = open_data_tcp(*p_, p_->host, err);
    if (!obn::plat::is_valid_socket(data_fd)) { err_out = err; return {}; }
    ...
    SSL* data_ssl = wrap_data_tls(*p_, p_->ctrl_ssl, data_fd, p_->host, err);
    if (!data_ssl) { obn::plat::close_socket(data_fd); err_out = err; return {}; }
...
void Client::quit()
{
    ...
    if (p_->ctrl_ssl) {
        ...
        // shutdown TLS.
        if (obn::plat::is_valid_socket(p_->ctrl_fd)) {
    ...
    if (obn::plat::is_valid_socket(p_->ctrl_fd)) {
        obn::plat::close_socket(p_->ctrl_fd);
