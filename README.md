...
H.264, decodes it, and re-encodes each frame as MJPEG so Studio's existing
`gstbambusrc` / `wxMediaCtrl2` consumers see one wire format regardless of
printer model. Pick the implementation at configure time:
...
> **Why no GStreamer backend any more?** An earlier revision shipped a
> GStreamer `rtspsrc → rtph264depay → h264parse → <h264 dec> →
> videoconvert → videoscale → jpegenc → appsink` pipeline as the
> default. It worked fine on a bare Linux desktop but always aborted
> inside Studio's AppImage with `Wrong JPEG library version: library
> is 80, caller expects 62` -- Studio's bundled GStreamer plugins
> were built against libjpeg-turbo ABI 62 while the system libjpeg
> is ABI 80, and we could not work around the conflict from outside
> Studio's bundle. The FFmpeg backend now in place avoids the issue
> entirely (it doesn't link or `dlopen` GStreamer at all), and it
> works on macOS too where GStreamer.framework was a non-starter.

The C-ABI consumer (gstbambusrc on Linux/Windows, `BambuPlayer.mm` on macOS)
sees the same JPEG bytes; the backend is only visible in `obn-bambusource.log`
("ff: ..." / "rtsp: ..." / "h264: ..." / "libav: ..." line tags) and in
the set of shared libraries the dylib pulls in (none, in the FFmpeg case).
...
runs inside the camera / file-browser code path that Studio's
`gstbambusrc` (Linux/Windows) or `wxMediaCtrl2.mm` (macOS) drives. It
keeps its own log mirror so you can see RTSP / TLS / FFmpeg failures
even when the parent Studio process funnels its log into something
unhelpful (e.g. `GST_DEBUG`).
...
| `OBN_JPEG_QUALITY`           | `80`                                                                                                                                                                                             | JPEG quality knob for the per-frame stb_image_write encoder, in `[1..100]`. The default mirrors what the legacy GStreamer `jpegenc` produced; raise for less compression artefacts at the cost of bigger frames over the C-ABI. |
