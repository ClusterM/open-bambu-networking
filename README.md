...
Our **FTPS workaround** in `libBambuSource.so` covers **external USB** browsing
only (P2S: FTPS root = USB stick when inserted). Stock keeps **TLS :6000** open
to firmware for the whole session — including the **Internal** tab — and does
**not** use FTPS :990 for file browser traffic on P2S. Details:
[NETWORK_PLUGIN.md §7.5.1](NETWORK_PLUGIN.md#751-where-the-printer-side-bytes-actually-come-from),
[STATUS.md — PrinterFileSystem (MediaFilePanel)](STATUS.md#printerfilesystem-mediafilepanel).

| Feature                                       | Status             | Impl       | Notes                                                                                                                                                                       |
| --------------------------------------------- | ------------------ | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| External storage browse (USB / SD)            | ✨ (tested on P2S) | Workaround | FTPS `LIST` / `RETR` / `DELE`; USB must be mounted on P2S. Stock port-6000 CTRL wire not reimplemented.                                                                      |
| Internal storage browse (eMMC timelapses)     | ❌                 | —          | Stock :6000 to firmware; FTPS never exposes eMMC on P2S.                                                                                                                    |
| `req.storage: "internal"` (Internal tab)      | ❌                 | —          | Ignored in [`handle_list_info`](stubs/BambuSource.cpp).                                                                                                                      |
| Download from external storage                | ✨ (tested on P2S) | Workaround | Streaming FTPS `RETR` with 256 KB `CONTINUE` chunks.                                                                                                                        |
| Delete from external storage                    | ✨ (tested on P2S) | Workaround | FTPS `DELE` per path.                                                                                                                                                       |
| 3MF plate thumbnails (`Metadata/plate_*.png`) | ⚠️                 | Workaround | Opens the archive via FTPS `RETR`, inflates the plate PNG with zlib. Flaky on very large archives. External paths only.                                                     |
| Timelapse / video thumbnails (external)       | ✨ (tested on P2S) | Workaround | Sidecar `<stem>.jpg` over FTPS; P2S may omit sidecars — see [STATUS.md](STATUS.md#printerfilesystem-mediafilepanel). Does not cover internal `.avi` on eMMC.               |
...
| `ipcam.file` block injection                                   | Firmware with a file browser advertises `ipcam.file = {"local":"local",…}`; Studio keys the MediaFilePanel off that.                                                                                                                                                                                                                                                                                                                                 | When the printer omits the block we inject `{"local":"local","remote":"none","model_download":"enabled"}` into the ipcam object. Present blocks pass through unchanged.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Without this Studio short-circuits MediaFilePanel with "Browsing file in storage is not supported in current firmware" and never calls `Bambu_StartStreamEx(CTRL_TYPE)` on us.                                                                                                                                                                                                                                    | Enables the panel and **External** tab over FTPS when USB is mounted; **Internal** tab (eMMC) stays empty vs stock — stock uses :6000 to firmware for internal, which we do not implement.                                                                                                                                                            |
| PrinterFileSystem CTRL bridge                                  | Port-6000 proprietary CTRL protocol served by the stock `libBambuSource.so`: TLS :6000 to firmware for lists, thumbnails, and downloads (no FTPS on P2S during file browse).                                                                                                                                                                                                                                                                          | In our `libBambuSource.so` a worker thread per tunnel answers `LIST_INFO` / `SUB_FILE` / `FILE_DOWNLOAD` / `FILE_DEL` / `REQUEST_MEDIA_ABILITY` / `TASK_CANCEL` by translating each into FTPS (`LIST`, `RETR`, `DELE`) after closing the :6000 socket. JSON is handled in-process.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | FTPS works for LAN print upload and external USB; documented Bambu FTPS quirks apply. The :6000 firmware wire is not documented.                                                                                                                                                                                                                                                                                                                 | **External USB only** on P2S. Internal eMMC not on FTPS (empty root without USB). `req.storage=="internal"` ignored. Future parity requires reverse-engineering stock's :6000 wire, not FTPS path mapping. `FILE_UPLOAD` not implemented (Studio uses `ft_*`).                                                                                      |
...
| `--disable-ftps-fastpath` | enabled                                                        | Disable `OBN_FT_TUNNEL_LOCAL`; Studio falls back to internal FTP send (`start_send_gcode_to_sdcard`). See [STATUS.md § 6.14](STATUS.md#614-file-transfer-abi-ft_). |
